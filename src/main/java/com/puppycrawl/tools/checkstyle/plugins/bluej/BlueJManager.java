...
public final class BlueJManager
{
    ...
    public Stage getCurrentFrame()
    {
        try
        {
            ...
            return mBlueJ.getCurrentPackage().getWindow();
        }
        catch (Exception e)
        {
            return mBlueJ.getCurrentWindow();
        }
    }
        ...
        String name, String defaultName, String description)
    {
        if (!mMissingResources.contains(name))
        {
            ...
            alert.setTitle("Checkstyle Extension");
//            alert.setHeaderText("Look, a Warning Dialog");
            alert.setContentText("Cannot find specified " + description + ":\n"
                    + "  \"" + name + "\"\nas a file, on the classpath, "
                    + "or as a URL."
                    + ((defaultName != null)
                    ? ("\nUsing default: " + defaultName)
...
//            final Frame bluejFrame = getCurrentFrame();
            // TODO: check below changes
            final Stage bluejFrame = getCurrentFrame();
            Point corner = new Point(0, 0);
            if (bluejFrame != null) {
//                corner = bluejFrame.getLocation();
                corner = new Point((int) bluejFrame.getMinWidth(), (int) bluejFrame.getMinHeight());
            }
